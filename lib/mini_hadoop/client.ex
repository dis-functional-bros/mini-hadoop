defmodule MiniHadoop.Client do
  ...
  defp replicate_block(block_id, block_data, datanodes) do
    replication_results = Enum.map(datanodes, fn datanode_hostname ->
      store_block_on_datanode(datanode_hostname, block_id, block_data)
    end)
  ...
  # FIXED: Handle hostnames instead of PIDs
  defp store_block_on_datanode(datanode_hostname, block_id, block_data) do
    datanode_node = String.to_atom(datanode_hostname)
  ...
  defp assemble_file_from_blocks(file_info) do
    block_results = Enum.map(file_info.blocks, fn {block_id, datanode_hostnames} ->
      read_block_with_retry(block_id, datanode_hostnames, @max_read_retries)
    end)
  ...
  defp read_block_from_nodes(block_id, [datanode_hostname | rest]) do
    datanode_node = String.to_atom(datanode_hostname)
